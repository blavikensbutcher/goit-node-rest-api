...
export async function addUser(email, password, subscription, avatarURL, verificationToken) {
  return User.create({ email, password, subscription, avatarURL, verificationToken });
}
...
export async function updateUserVerification(userID, verificationToken, verify) {
  return User.findByIdAndUpdate(
      userID,
